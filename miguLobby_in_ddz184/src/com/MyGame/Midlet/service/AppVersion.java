...
public class AppVersion {
	...
	public AppVersion(Context context, XmlPullParser p)
	{
		...
		if(Configs.isMediaMounted())
		{
			try
			{
				final File iconFile = new File(Configs.getSdcardPath() + AppConfig.ICONS_PATH, iconName);
				final File iconFiletmp = new File(Configs.getSdcardPath() + AppConfig.ICONS_PATH, iconName+".tmp");
				if(iconFile.exists() && iconFile.isFile())
				{
					iconDrawable = Configs.getDrawableFromFile(context,iconFile,DisplayMetrics.DENSITY_HIGH);
		...
		apkFilePath=Configs.getSdcardPath() + AppConfig.APKS_PATH+"/"+apkFileName;

		if(Configs.isEmptyStr(onLineNum)){
			new Thread(){
				@Override
				public void run(){
					String userdata=Configs.getConfigXmlByHttp(getOnLineUrl());
	...
	public Drawable getGameIcon(){
		if(iconDrawable==null){
			Bitmap iconBitmap=BitmapFactory.decodeResource(mContext.getResources(), R.drawable.game_default_icon);
	...
	public boolean isUpdateComplete(){
		...
		{
			File apkFile = new File(Configs.getSdcardPath() + AppConfig.APKS_PATH, apkFileName);
	...
	public String getOnLineNum(){
		if(id<0){
			return "即将上线";
		}else if(Configs.isEmptyStr(onLineNum)||onLineNum.trim().equals("0")){
			return onLineNum;
		}else{
			return onLineNum+"人";
		}
